...
\begingroup\setlength\intextsep{-18pt}% hack for fixing space above the figure.
...
\end{wrapfigure}
    ...
    only that people be within a certain distance of each other.
}
...
\begin{wrapfigure}[7]{r}{0.38\linewidth}
    \begin{center}\begin{tikzpicture}[scale=1.1]%{{{
        ...
        \node[draw, circle, fill=uofgblue40, inner sep=5pt, font=\bfseries] (N1) at (4.5,  0) {1};
        \node[draw, circle, fill=white, inner sep=5pt, font=\bfseries] (N2) at (6.5,  0) {2};
        \node[draw, circle, fill=uofgblue, inner sep=5pt, font=\bfseries] (N3) at (4.5, -2) {3};
        \node[draw, circle, fill=white, inner sep=5pt, font=\bfseries] (N4) at (6.5, -2) {4};
        \node[draw, circle, fill=uofgtorange, inner sep=5pt, font=\bfseries] (N5) at (4.5, -4) {5};
        \node[draw, circle, fill=uofgtlightgreen, inner sep=5pt, font=\bfseries] (N6) at (6.5, -4) {6};
...
\block{More Flexibility via Constraint Programming}{
    These techniques can be used for \emph{any} system where we have some variables, and
    some constraints between those variables, and we wish to instantiate the variables whilst
    respecting all the constraints. This is called \textbf{constraint programming}.
...
operations, designing a national railway system, reducing the cost of an invasion, and organising
sporting seasons for optimal TV coverage. Constraint programming is also extremely good for solving
Sudoku puzzles.

\medskip

\begin{center}
    \includegraphics*[keepaspectratio=true,height=6.5cm]{train.jpg}
    \hfill
    \includegraphics*[keepaspectratio=true,height=6.5cm]{comet.jpg}
    \hfill
    \includegraphics*[keepaspectratio=true,height=6.5cm]{airport.jpg}
\end{center}
}
...
\block[bodyverticalshift=0cm]{}{
    ...
    This work was
    supported by the Engineering and Physical Sciences Research Council [grant number EP/K503058/1].
